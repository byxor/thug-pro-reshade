...
float3 GetNormalFromDepth(float fDepth, float2 vTexcoord) {
  	...
  	float depth1 = tex2Dlod(ReShade::LinearizedDepth, float4(vTexcoord + offset1,0,0)).x;
  	float depth2 = tex2Dlod(ReShade::LinearizedDepth, float4(vTexcoord + offset2,0,0)).x;
...
void PS_AO_SSAO(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 Occlusion1R : SV_Target0)
{
	...
	//global variables
	float fSceneDepthP 	= tex2D(ReShade::LinearizedDepth, texcoord.xy).x;
	...
	if(fSceneDepthP > min(0.9999,AO_FADE_END)) Occlusion1R = float4(0.5,0.5,0.5,blurkey);
		...
		for (int z = -1 ; z <= 1 ; z += 2) {
			...
			float fSceneDepthS = tex2Dlod(ReShade::LinearizedDepth, float4(vSamplePos.xy,0,0)).x;
...
void PS_AO_RayAO(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 Occlusion1R : SV_Target0)	
{
	...
	fCurrDepth  = tex2D(ReShade::LinearizedDepth, texcoord.xy).x;
	...
	if(fCurrDepth>min(0.9999,AO_FADE_END)) Occlusion1R = float4(1.0,1.0,1.0,blurkey);
		...
		for(int s = 0; s < iRayAOSamples; s++) {
			...
			fSampleDepth = tex2Dlod(ReShade::LinearizedDepth, float4(saturate(texcoord.xy + (fRayAOSamplingRange * vReflRay.xy / fCurrDepth)),0,0)).x;
...
void PS_AO_HBAO(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 Occlusion1R : SV_Target0)		
{
	...
	float depth = tex2D(ReShade::LinearizedDepth, texcoord.xy).x;
	...
	if(depth > min(0.9999,AO_FADE_END)) Occlusion1R = float4(1.0,1.0,1.0,blurkey);
		...
		for (int i = 0; i < 8; i++)
		{
			...
			{
				sample_depth = tex2Dlod(ReShade::LinearizedDepth, float4(sample_center + sample_coords*(k-0.5*(i%2)),0,0)).x;
...
{
	float z = tex2Dlod(ReShade::LinearizedDepth, float4(uv, 0, 0)).x; // Single channel zbuffer texture
...
{
    	float3 Position;
	Position.z = GetSAO_CurveDepth(tex2Dlod(ReShade::LinearizedDepth, float4(ssPosition.xy,0,0)).x);
...
float3 GetSAO_OffsetPosition(float2 ssC, float2 unitOffset, float ssR)
{
	...
	float3 P;
	P.z = GetSAO_CurveDepth(tex2Dlod(ReShade::LinearizedDepth, float4(ssP.xy,0,0)).x);
...
void PS_AO_SSGI(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 Occlusion1R : SV_Target0)	
{
	...
	if(depth > 0.9999) Occlusion1R = float4(0.0,0.0,0.0,1.0);
		...
		for (int i = 0; i < iSSGISamples; i++) {
			...
			float  sample_depth = rangeZ * tex2Dlod(ReShade::LinearizedDepth,float4(sample_coords.xy,0,0)).x;
			...
			if ((sample_depth < ii_sample_center_depth + ii_curr_sample_radius) &&
				...
				float3 unit_vector = normalize(pos - sample_pos);
 				gi.rgb += tex2Dlod(ReShade::OriginalColor, float4(sample_coords,0,0)).rgb;
			}
...
void PS_AO_GIBlurV(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 Occlusion2R : SV_Target0) 
{
	...
	float totalweight=0;
	float4 base = tex2D(SamplerOcclusion1, texcoord.xy), temp = 0;
	float depth = tex2Dlod(ReShade::LinearizedDepth, float4(texcoord.xy,0,0)).x;
#if( AO_SHARPNESS_DETECT == 1)
	float blurkey = depth;
#else
	float blurkey = dot(GetNormalFromDepth(depth, texcoord.xy).xyz,0.333)*0.1;
