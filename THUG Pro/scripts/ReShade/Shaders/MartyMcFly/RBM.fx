...
{
	return 202.0 / (-99.0 * tex2Dlod(ReShade::OriginalDepth, float4(coords.xy,0,0)).x + 101.0);
}
...
{
	float4 res = tex2D(ReShade::BackBuffer, texcoord.xy);
	float lineardepth = tex2D(ReShade::LinearizedDepth, texcoord.xy).r;
	res.a = lineardepth;
	hdrR = res;
}
...
float4 PS_RBM_Execute(float4 vpos : SV_Position, float2 texcoord : TEXCOORD) : SV_Target
{
	...
	float3 bumpNormals   = clamp(lerp(float3(0.0,0.0,1.0),GetBumpNormals(ReShade::BackBuffer, texcoord.xy),fReflectionReliefHeight),-1.0,1.0);
