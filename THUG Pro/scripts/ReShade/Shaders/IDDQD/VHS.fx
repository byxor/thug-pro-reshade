...
void PS_VHS(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 GetOut : SV_Target0)
{	
	float4 origcolor;
...
#define t ReShade::Timer.x*0.001
#define t2 ReShade::Timer*0.001
...
float nn(float2 p){
    ...
    float v = (n( float3(y*0.01 +s, 			1., 1.0) ) + .0)
    ...
    v += 0.3;
	if(v<0.7) v = 0.0;  //threshold
    return v;
}

void PS_VHS3(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 OutGet : SV_Target0)
{
    float4 uv = 0.0;
	uv.xy = texcoord.xy;
	uv.w = 0.0;
...
void PS_VHS6(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 OutGet3 : SV_Target0)
{
